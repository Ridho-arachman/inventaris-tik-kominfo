...
export default function EditHardwareForm() {
  ...
  const { loading, put } = usePut(`/hardware-opd/${id}`);

  const {
    data: hardware,
    error: hardwareError,
    isLoading: hardwareLoading,
  } = useGet(`/hardware-opd/${id}`);

  const { data: kategori } = useGet("/hardware/kategori");

  const form = useForm<z.infer<typeof hardwareOpdSchema>>({
    resolver: zodResolver(hardwareOpdSchema),
    defaultValues: {
      nama: "",
      merk: "",
      spesifikasi: "",
      ...
      sumber: SumberPengadaan.PEMBELIAN,
    },
  });
  ...
  async function onSubmit(values: z.infer<typeof hardwareOpdSchema>) {
    try {
      ...
      router.push("/opd/asset/hardware");
    } catch (error) {
