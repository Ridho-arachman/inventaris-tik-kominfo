...
import { kategoriHardwareSchema } from "@/schema/ketegoriHardwareSchema";
...
export default function EditKategoriHardwarePageUI() {
  ...
  const { data, isLoading, error } = useGet(`/hardware/kategori/${id}`);
  const { loading, put } = usePut(`/hardware/kategori/${id}`);

  const form = useForm<z.infer<typeof kategoriHardwareSchema>>({
    resolver: zodResolver(kategoriHardwareSchema),
  ...
  const onSubmit = async (values: z.infer<typeof kategoriHardwareSchema>) => {
    try {
              ...
              <Controller
                ...
                render={({ field, fieldState }) => (
                  <Field data-invalid={fieldState.invalid}>
                    ...
                    <FieldDescription>
                      Masukkan nama kategori hardware.
                    </FieldDescription>
