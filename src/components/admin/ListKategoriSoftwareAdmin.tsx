...
export default function KategoriSoftwareListComponent() {
  ...
  const {
    ...
    error,
    mutate,
  } = useGet(`/software/kategori?q=${debounced}`);
  ...
  const handleDelete = async (id: string) => {
    try {
      const res = await del(`/software/kategori/${id}`);
      notifier.success(
        "Berhasil",
        `Kategori Software ${res.data.nama} Berhasil Dihapus`
      );
      mutate(`/software/kategori?q=${debounced}`);
    } catch (error) {
      const err = error as AxiosError<ApiError>;
      notifier.error(
        "Gagal Menghapus Kategori Software",
  ...
  return (
    <div className="min-h-screen p-6 bg-gray-50">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
        <h1 className="text-3xl font-bold text-gray-800">Kategori Software</h1>
        ...
        <div className="flex items-center gap-3">
          ...
          <Button asChild>
            <Link href="/admin/manage-kategori-asset/software/add">
              <Plus className="w-4 h-4 mr-1" />
              Tambah Kategori Software
            </Link>
          </Button>
        </div>
      </div>
      ...
      <div className="mb-8 flex items-center gap-3 max-w-md">
        <div className="relative w-full">
          ...
          <Input
            ...
            placeholder="Cari kategori software..."
                ...
                <Card className="shadow-md hover:shadow-lg transition-all border border-gray-200">
                  ...
                  <CardContent className="flex flex-col gap-3">
                    ...
                    <div className="flex items-center justify-end gap-3 pt-4 border-t">
                      ...
                      >
                        <Link
                          href={`/admin/manage-kategori-asset/software/${kategori.id}`}
                        >
